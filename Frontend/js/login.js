...
async function submitStep1(){
  ...
  try {
    ...
    if(data.role !== selectedRole){
      showToast(`This account belongs to ${data.role}`);
      return;
    }
    ...
    sessionStorage.setItem("email", uid);
...
async function submitStep2(){
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/auth/verify-role`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token: token, roleCode: roleCode })
    });

    const data = await res.json();

    if(!res.ok){
      showToast(data.message || "Wrong role code");
      return;
    }
